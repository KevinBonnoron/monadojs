...
export const isEmpty = <T>(value: unknown): value is Required<NoUndefinedField<T>> => (isNil(value) ? true : isPrimitive(value) ? true : isFunction(value) ? true : isArray(value) ? value.length === 0 : isDate(value) ? !isNaN(Number(value)) : isRegExp(value) ? false : isObject<any>(value) ? Object.keys(value).length === 0 : false);
