...
export function valuesOf(source: unknown): IterableIterator<unknown> | undefined {
  return isArray(source) ? [...source].values() : isSet(source) ? [...source].values() : isMap(source) ? source.values() : isPlainObject(source) ? Object.values(source as Record<string, unknown>).values() : undefined;
}
