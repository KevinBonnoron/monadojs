...
export function reduce<T, U extends T>(predicate: Reducer<T, U>, initialValue?: U) {
  return (values: T) => (isArray<T>(values) ? (values.length === 0 ? values : isUndefined(initialValue) ? values.slice().reduce(predicate) : values.slice().reduce(predicate, initialValue)) : predicate(values, initialValue as T, 1, [values]));
}
