...
import { DIVERSE_ARRAY, DIVERSE_MAP, DIVERSE_SET, NUMBER_ARRAY, NUMBER_MAP, NUMBER_SET, STRING_ARRAY, STRING_MAP, STRING_SET } from '../../../tests/test.data';
import { eq } from '../../filters';
import { Just, Nothing } from '../../utils';
import { number } from '../../wrappers';
...
describe('match', () => {
  describe('Array', () => {
    it('should return values that match with Maybe', () => {
      ...
      expect(operator(source)).toStrictEqual(['null', 'null', 'a', 1, 'just', 'just', 'just', 'just', 'just', 'just', 'just', 'just', 'just', 'just', 'just', 'just', 'null']);
    });
    ...
    it('should return values that match with operator', () => {
      const source = NUMBER_ARRAY;
      const operator = match([
        { if: pipe(number.modulo(3), eq(0)), then: 'zero' },
        { if: pipe(number.modulo(3), eq(1)), then: 'one' },
      ]);
    ...
    it('should return values that match with regex', () => {
      ...
      const expected = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing];
    ...
    it('should return values that match default', () => {
      const source = NUMBER_ARRAY;
      const operator = match([{ if: pipe(number.modulo(3), eq(0)), then: 'zero' }, { then: 'one' }]);
